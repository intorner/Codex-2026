...
>
> 例如未來做投票工具：
> 1. 編輯 `firestore.rules`，加上 `match /votes/{document} { allow read, write: if true; }`
...
> ⚠️ **2026-05-03 實測補充**：
> - 如果你用的是 **Claude Code**，可先照下面的 `claude mcp add` 做。
> - 如果你用的是 **Codex 桌面版**，實測上常常不是讀工作區 `.mcp.json`，而是讀 `C:\Users\你的帳號\.codex\config.toml`。
> - 換句話說，**Codex 使用者請優先檢查 `config.toml` 是否真的有 `[mcp_servers.firebase]`**，不要只看工作區 `.mcp.json`。
...
> 💡 **Windows 建議直接寫 `npx.cmd` 的完整路徑**：
> - 避免 PowerShell 把 `npx` 解析到 `npx.ps1`
> - 避免被 Execution Policy 擋下來
> - 這次實測可成功的設定是 `C:\Program Files\nodejs\npx.cmd`
...
如果你在 **Windows PowerShell** 遇到「因為這個系統上已停用指令碼執行，所以無法載入 npx.ps1」之類錯誤，請改用：

```powershell
& "C:\Program Files\nodejs\npx.cmd" -y firebase-tools@latest login
```
...
```powershell
& "C:\Program Files\nodejs\npx.cmd" -y firebase-tools@latest projects:list
```
...
> 💡 **實測判斷是否真的走 MCP**：
> - 如果對話顯示「已使用 Firebase」，通常就是 Firebase MCP 已載入成功
> - 如果它開始自己跑很多 CLI 指令、找 token、找憑證、繞 REST API，通常代表 **MCP 沒有真正載入**

> 如果連接失敗，請檢查：
> - 是否已完成 `firebase login`
> - Node.js 是否正常安裝
> - Codex 使用者是否真的把 Firebase MCP 寫進 `C:\Users\你的帳號\.codex\config.toml`
> - 重新執行步驟三和步驟四
...
1. 在 Firestore 建立一個測試集合：
   ...
   - **第一次測試先只寫 `message` 一個欄位，不要一開始就寫 `created_at = 現在時間`**
   - message 值：「Firebase 連接測試成功」

2. 查詢 `test_collection`，確認能讀取到剛才新增的資料

3. 如果第一步成功，再補一個 `created_at` 欄位
   - 建議先用字串，例如 `"2026-05-03T09:27:05+08:00"`
   - **不要第一次就用自然語言的「現在時間」測試**，否則容易踩到型別轉換問題

4. 刪除測試文件和集合

5. 告知使用者：「連接測試成功！Firebase 已準備好使用。」

> 💡 **2026-05-03 實測坑點**：
> - `firestore_add_document` / `firestore_update_document` 可成功寫入，但參數型別要很精確
> - `document` 不能傳 JSON 字串，必須傳原生物件
> - `updateMask` 不能傳 `"message,created_at"` 這種字串，必須傳：
>
> ```json
> {
>   "fieldPaths": ["message", "created_at"]
> }
> ```
>
> - 可成功的 `document` 格式例如：
...
```powershell
& "C:\Program Files\nodejs\npx.cmd" -y firebase-tools@latest logout
& "C:\Program Files\nodejs\npx.cmd" -y firebase-tools@latest login
```
...
| PowerShell 說 `無法載入 npx.ps1` / `因為這個系統上已停用指令碼執行` | 改用 `& "C:\Program Files\nodejs\npx.cmd" ...`，不要直接打 `npx` |
...
| 寫入失敗：Permission denied | Firestore 規則沒允許這個集合。到 Rules 加一段 `match /集合名/{document} { allow read, write: if true; }` |
| 寫入失敗：`Invalid value at 'document'` | `document` 很可能被傳成字串了。請改傳原生物件，不要傳 JSON 字串 |
| 更新失敗：`Invalid value at 'update_mask'` | `updateMask` 不能是逗號分隔字串，要傳 `{"fieldPaths":[...]}` |
...
| `collection id must be set when show_missing is true` | 查集合時如果開了 `showMissing=true`，就必須同時提供 `collectionId` |
...
| 重開後還是沒抓到 Firebase MCP | 如果你用的是 Codex，先檢查 `C:\Users\你的帳號\.codex\config.toml` 是否真的有 `[mcp_servers.firebase]`，不要只檢查工作區 `.mcp.json` |
...
| 2026-05-03 | v0.8 | 補上 Codex 實測流程：Windows 要用 `npx.cmd`、Codex 實際讀取 `config.toml`、測試規則需放行 `test_collection`、Firestore MCP 的 `document` 與 `updateMask` 需傳結構化物件 |
